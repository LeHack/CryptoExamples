 ...
 * Camellia in ECB mode with PKCS#7 padding
 *
 * Syntax:
 *  ./camellia_ecb [encrypt|decrypt] [input.file] [output.file]
 * Example:
 *  ./camellia_ecb encrypt myfile.doc myfile.doc.enc
 *  ./camellia_ecb decrypt myfile.doc.enc myfile.doc
 *
 * !!! WARNING !!!
 * This is a proof of concept. DO NOT use it to encode anything you actually want to protect.
 * See this link to understand why: https://blog.filippo.io/the-ecb-penguin/
 * For encrypting sensitive data please turn to more suitable tools like GPG.
 */
...
int decrypt(int infd, int outfd) {
    ...
    while (1) {
        if (n > 0) {
            ...
            Camellia_ecb_encrypt(inbuff, outbuf, &key, 0);
            bufready = 1;
        }
...
int encrypt(int infd, int outfd) {
    ...
    while (1) {
        ...
        Camellia_ecb_encrypt(inbuff, outbuf, &key, 1);
